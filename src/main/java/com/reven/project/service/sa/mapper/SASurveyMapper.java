...
public interface SASurveyMapper {
    /** 관리자 설문 관리 목록을 조회한다. */
    List<SADto.SurveyListItem> selectSurveyList(SADto.SurveySearchRequest request);
    ...
    List<SADto.SurveyField> selectSurveyFields(@Param("surveySeq") Long surveySeq);

    /** 설문 문항의 선택형 보기 목록을 조회한다. */
    List<SADto.SurveyOption> selectSurveyOptions(@Param("surveySeq") Long surveySeq);

    /** 설문 마스터를 등록한다. */
    void insertSurvey(SADto.SurveyDetail survey);

    /** 설문 마스터를 수정한다. */
    void updateSurvey(SADto.SurveyDetail survey);
    ...
    /** 설문 문항 보기를 등록한다. */
    void insertSurveyOption(SADto.SurveyOption option);
}
