...
This file captures project-specific UI, content, and workflow rules for the PHP survey intake site.
...
- Treat `data/forms.json` and `data/submissions.json` as local runtime data only.
...
- The app runs with Docker on `http://localhost:8000`.
- PHP pages are served from the `php:8.3-apache` container.
