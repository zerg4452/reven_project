...
- 코드 리팩토링(중복 제거)을 진행했다. `BDNoticeService`·`BDPhotoBoardService`에 복붙돼 있던 파일 디스크 저장·경로 해석·커밋 후 삭제 로직을 `service/bd/support/BDFileStorageSupport`로 추출하고(R1), 4개 서비스(`BDNotice`/`BDPhotoBoard`/`BDAiNews`/`COAdminMenu`)에 바이트 단위로 동일하던 `firstText(String...)`를 `common/util/TextUtils`로 통합했다(R2). 동작 보존 리팩토링이라 새 테스트는 추가하지 않고 기존 회귀로 검증했으며 두 BD 서비스 합쳐 약 150줄을 줄였다. `SASurveyDto` 분리는 churn 대비 이득이 낮아 보류(R3), `normalizedAdminSearch`는 컨트롤러가 쓰는 살아 있는 API라 유지(R4)로 결정했다. 별도 코드 리뷰에서 Critical/Important 0건·머지 가능 판정을 받고 `refactor/bd-dedup`를 main에 fast-forward 머지했다. 후속 테스트 갭 2건(`BDNoticeServiceTest` 파일 로직 미커버, `resolveStoredFilePath` 경로 탈출 음성 테스트 부재)은 별도 태스크로 분리했다. 완료 계획서는 `docs/clear/refactor/2026-06-07-bd-refactor-dedup-plan.md`. `./gradlew test` 전체 통과(150 tests).
